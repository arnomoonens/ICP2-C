...
static _NIL_TYPE_ IMR(_NIL_TYPE_);                              //TOEGEVOEGD
static _NIL_TYPE_ Imr(_NIL_TYPE_);                              //TOEGEVOEGD
static _NIL_TYPE_ OCK(_NIL_TYPE_);                              //TOEGEVOEGD
     ...
     { siz = _ag_get_NBU_(nbr);
       if (siz == 0)
         ...
         { _mem_claim_SIZ_(siz);
           ...
           _ag_set_TAB_OWN_(tab, _VOID_);                                                  //TOEGEVOEGD
           _stk_peek_EXP_(exp);
           _stk_poke_EXP_(tab);
           _stk_push_EXP_(exp);
...
/*     cont-stack: [... ... ... ... ... SET] -> [... ... ... ... ... ...] */
/*                                                                        */
/*     expr-stack: [... ... ... ... ... SET] -> [... ... IDX EXP MOD TAB] */
/*     cont-stack: [... ... ... ... ... SET] -> [... ... ... ... ... OCK] */
/*------------------------------------------------------------------------*/
static _NIL_TYPE_ SET(_NIL_TYPE_)
   ...
   switch (tag)
    { case _VAR_TAG_:
      ...
      case _TBL_TAG_:
        ...
        idx = _ag_get_TBL_IDX_(inv);
        _dct_locate_(nam, dct, _DCT_);
        tab = _ag_get_DCT_VAL_(dct);                                            //TOEGEVOEGD
        _stk_poke_EXP_(idx);
        _stk_push_EXP_(exp);
        _stk_push_EXP_(_CRO_);
        _stk_push_EXP_(tab);
        _stk_poke_CNT_(OCK);
        break;
      default:
...
/*------------------------------------------------------------------------*/
/*  OCK                                                                   */
/*     expr-stack: [... ... IDX EXP MOD TAB] -> [... ... IDX EXP MOD TAB] */
/*     cont-stack: [... ... ... ... ... OCK] -> [... ... ... ... ... OCK] */
/*                                                                        */
/*     expr-stack: [... ... IDX EXP MOD TAB] -> [... ... ... TAB IDX EXP] */
/*     cont-stack: [... ... ... ... ... OCK] -> [... ... ... RPL SWP EXP] */
/*------------------------------------------------------------------------*/
static _NIL_TYPE_ OCK(_NIL_TYPE_)                                               //TOEGEVOEGD
 { _EXP_TYPE_ tab, cro, exp, idx;
   _stk_pop_EXP_(tab);
   _stk_pop_EXP_(cro);
   if(_mem_is_same_(_ag_get_TAB_OWN_(tab), cro)) {
       _stk_pop_EXP_(exp);
       _stk_pop_EXP_(idx);
       _stk_push_EXP_(tab);
       _stk_push_EXP_(idx);
       ...
       _stk_push_CNT_(SWP);
       _stk_push_CNT_(EXP);
   } else if(_ag_is_VOI_(cro)) {
       _error_(_IMT_ERROR_);
   } else {
       _stk_push_EXP_(_ag_get_MOD_OWN_(cro));
       _stk_push_EXP_(tab);
   }
 }
...
/*------------------------------------------------------------------------*/                            //TOEGEVOEGD
/*  IMR                                                                   */
/*     expr-stack: [... ... ... ... ... MRF] -> [... ... ... DCT MOD INV] */
/*     cont-stack: [... ... ... ... ... IMR] -> [... ... ... ... Imr EXP] */
/*------------------------------------------------------------------------*/
static _NIL_TYPE_ IMR(_NIL_TYPE_)
 { _EXP_TYPE_ mrf, mod, inv, dct, fnd, nam, hold, dcx, new;
   _TAG_TYPE_ tag;
     _stk_claim_();
     _mem_claim_();
     _stk_peek_EXP_(mrf);
     mod = _ag_get_MRF_MOD_(mrf);
     _dct_locate_(mod, fnd, _DCT_); //Zoek gebruikte module op in globale dict.
     mod = _ag_get_DCT_VAL_(fnd);
     if (_ag_get_TAG_(mod) != _MOD_TAG_) {
         _error_(_MOD_ERROR_); }
     dct = _ag_get_MOD_DCT_(mod);
     
     inv = _ag_get_MRF_INV_(mrf);
     tag = _ag_get_TAG_(inv);
     //Haal de naam uit de invocatie...
     if (tag == _VAR_TAG_) {
         nam = _ag_get_VAR_NAM_(inv);
     } else if (tag == _TBL_TAG_) {
         nam = _ag_get_TBL_NAM_(inv);
     } else if (tag == _APL_TAG_) {
         nam = _ag_get_APL_NAM_(inv);
     } else if (tag == _MRF_TAG_) {
         nam = _ag_get_MRF_MOD_(inv);
     }
     _dct_locate_(nam, fnd, dct); //En zoek ze op in de dictionary van de module

     hold = _CRO_;
     _CRO_ = mod; //Sla huidige module globaal op als huidige owner
     new = _ag_make_DCT_();
     dcx = _DCT_;
     _ag_set_DCT_NAM_(new, nam);
     _ag_set_DCT_VAL_(new, _ag_get_DCT_VAL_(fnd));
     _ag_set_DCT_DCT_(new, _DCT_);
     _DCT_ = new;
     //Voor invocatie uit en zet daarna de huidige owner terug
     _stk_poke_EXP_(dcx);
     _stk_push_EXP_(hold);
     _stk_push_EXP_(inv);
     _stk_poke_CNT_(Imr);
...
/*------------------------------------------------------------------------*/                            //TOEGEVOEGD
/*  Imr                                                                   */
/*     expr-stack: [... ... ... DCT MOD EXP] -> [... ... ... ... ... EXP] */
/*     cont-stack: [... ... ... ... ... Imr] -> [... ... ... ... ... ...] */
/*------------------------------------------------------------------------*/
static _NIL_TYPE_ Imr(_NIL_TYPE_)
 { _EXP_TYPE_ exp;
   _stk_pop_EXP_(exp);
   _stk_pop_EXP_(_CRO_); //Zet huidige owner terug (hold van in IMR functie)
   _stk_peek_EXP_(_DCT_);
   _stk_poke_EXP_(exp);
   _stk_zap_CNT_(); }

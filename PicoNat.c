...
static _NIL_TYPE_ ERU(_NIL_TYPE_);
   /*--MODULE--*---*/                                                           //TOEGEVOEGD
static _NIL_TYPE_ MOD(_NIL_TYPE_);
static _NIL_TYPE_ MOd(_NIL_TYPE_);
...
/*------------------------------------------------------------------------*/                            //TOEGEVOEGD
/*  MOd                                                                   */
/*     expr-stack: [... ... ... TAB DCT NBR] -> [... ... ... TAB DCT NBR] */
/*     cont-stack: [... ... ... ... ... MOd] -> [... ... ... ... ... MOd] */
/*                                                                        */
/*     expr-stack: [... ... ... TAB DCT NBR] -> [... ... ... ... ... MOD] */
/*     cont-stack: [... ... ... ... ... MOd] -> [... ... ... ... ... ...] */
/*------------------------------------------------------------------------*/
static _NIL_TYPE_ MOd(_NIL_TYPE_)
 { _EXP_TYPE_ nbr, dct, arg, exp, fnd, nam, ndt, val, mod;
   _UNS_TYPE_ idx, siz;
   _TAG_TYPE_ tag;
   _mem_claim_(); //Voor maken DCT, IMT, MOD
   _stk_pop_EXP_(mod);
   _stk_pop_EXP_(nbr);
   _stk_pop_EXP_(dct);
   _stk_peek_EXP_(arg);
   idx = _ag_get_NBU_(nbr);
   siz = _ag_get_TAB_SIZ_(arg);
   if (idx <= siz) { //Nog niet elk argument is verwerkt
       exp = _ag_get_TAB_EXP_(arg, idx);
       tag = _ag_get_TAG_(exp);
       if (tag == _VAR_TAG_) { //Alleen variabelen toegelaten
           nam = _ag_get_VAR_NAM_(exp);
           _dct_locate_(nam, fnd, _DCT_);
           val = _ag_get_DCT_VAL_(fnd);
           tag = _ag_get_TAG_(val);
           ndt = _ag_make_DCT_(); //Nieuwe dict om aan 'oude' toe te voegen
           _ag_set_DCT_NAM_(ndt, nam);
           if (tag == _TAB_TAG_ && _mem_is_same_(_ag_get_TAB_OWN_(val), _VOID_)) { //Maak van een gewone tabel een immutable tabel
               _ag_set_TAB_OWN_(val, mod);
           } else if (tag == _MOD_TAG_ && _mem_is_same_(_ag_get_MOD_OWN_(val), _VOID_)) {
               _ag_set_MOD_OWN_(val, mod);
           }
           _ag_set_DCT_VAL_(ndt, val);
           _ag_set_DCT_DCT_(ndt, dct);
           dct = ndt;
           _stk_push_EXP_(dct);
           _stk_push_EXP_(_ag_succ_NBR_(nbr));
           _stk_push_EXP_(mod);
       } else {
           _error_str_(_ATC_ERROR_, mod_STR);
    } } else { //Alle argumenten verwerkt: maak van het geheel een MOD
        _ag_set_MOD_DCT_(mod, dct);
        _ag_set_MOD_OWN_(mod, _VOID_);
        _stk_poke_EXP_(mod);
        _stk_zap_CNT_();
    }
}
